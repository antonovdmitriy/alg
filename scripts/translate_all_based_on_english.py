...
async def translate_from_en_async(limiter, en_text):
    messages = [
        {
            "role": "system",
            "content": (
                "You are a multilingual translator. You will be given a phrase in English. "
                "Translate it into the following languages (excluding English): "
                + ", ".join([lang for lang in TARGET_LANGS if lang != "en"]) +
    ...
    try:
        ...
        print(f"❌ Ошибка при переводе '{en_text}': {e}")
        return {}

async def process_entry(limiter, entry):
    en_word = entry.get("translations", {}).get("en", "")
    print(f"🔍 Слово: '{entry.get('word')}', en: '{en_word}'")
    if en_word:
        translated = await translate_from_en_async(limiter, en_word)
...
async def process_category(limiter, category):
    print(f"📂 Translating full category: {category.get('id')} | title: {category.get('title')} | en: {category.get('translations', {}).get('en')}")
    en_cat = category.get("translations", {}).get("en", "")
    if en_cat:
        translated_cat = await translate_from_en_async(limiter, en_cat)
...
async def translate_category_only(category, limiter):
    ...
    en_cat = category.get("translations", {}).get("en", "")
    print(f"🔤 Source: en='{en_cat}'")
    if not en_cat:
        print("⚠️ Пустой en_cat, перевод не будет выполнен")
    if en_cat:
        translated_cat = await translate_from_en_async(limiter, en_cat)
...
if __name__ == "__main__":
    ...
    if args.id:
        ...
        if entry:
            ...
            if en_text:
                translated = asyncio.run(translate_from_en_async(AsyncLimiter(1,2), en_text))
                entry["translations"] = merge_translations(entry.get("translations", {}), translated)
            else:
                print("⚠️ Нет английского перевода у записи")
        else:
